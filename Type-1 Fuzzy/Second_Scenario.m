...
for iteration=1:10
    vacc = mamfis("NumInputs", 2, "NumOutputs", 2, 'NumInputMFs', 2 , 'NumOutputMFs', 3, 'MFType', "gaussmf"); %,'NumInputMFs',5,'NumOutputMFs',5
    vacc.Inputs(1,1).Range = [-0.25 0.25];
    vacc.Inputs(1,2).Range = [-0.05 0.05];
    vacc.Outputs(1,1).Range = [0 0.5];
    vacc.Outputs(1,2).Range = [0 0.5];

    options = tunefisOptions("Method","ga",'OptimizationType', "learning");
    options.MethodOptions.MaxGenerations = 1000;
    options.MethodOptions.PopulationSize = 1000;
    ...
    save('result/second_scenario_optimization_'+string(iteration)+'q5_ka25.mat')
    %% test
    I_un_std = [];
    x_std = [];
    I_std = [];
    ...
    for i=2:60
        ...
        evalfis_result = evalfis(fisTout1,input_vector);
        vacci_rate(i) = max(0,evalfis_result(1));
        isola_rate(i) = max(0,evalfis_result(2));
...
for i=2:60
    ...
    input_vector = [I(i) - y(i) ((I(i) - y(i))-(I(i-1) - y(i-1)))]/N(i);
    input_vector(abs(input_vector)<0.01) = 0;
    evalfis_result = evalfis(vacc,input_vector);
    vacci_rate(i) = max(0,evalfis_result(1));
    isola_rate(i) = max(0,evalfis_result(2));
end
