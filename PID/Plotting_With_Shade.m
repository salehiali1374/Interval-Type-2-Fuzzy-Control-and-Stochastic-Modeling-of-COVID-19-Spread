...
for iteration=1:10
    %% test
    load('first_scenario_optimization_'+ string(iteration) + 'q5_ka54.mat');
    x = 1:1:60;
    y = 0.1*ones(1,60);
    ...
    for i=2:60
        ...
        input_vector = [I(i) - y(i) ((I(i) - y(i))-(I(i-1) - y(i-1)))]/N(i);
        input_vector(abs(input_vector)<0.01) = 0;
        evalpid_result = evalpid(P,input_vector);
        vacci_rate(i) = max(0,evalpid_result(1));
        isola_rate(i) = max(0,evalpid_result(2));
...
I_un_curve2 = I_un_mean - I_un_std;

I_curve1 = I_mean + I_std;
I_curve2 = I_mean - I_std;

y_curve1 = y_mean + y_std;
y_curve2 = y_mean - y_std;
...
hold on
plot(vacci_rate,'-.', 'LineWidth',2);
plot(isola_rate, '-s', 'LineWidth',2);


% shade(x,I_un_curve1,x,I_un_curve2,'FillType',[1 2;2 1]);
% shade(x,I_curve1,x,I_curve2,'FillType',[1 2;2 1]);
% shade(x,y_curve1,x,y_curve1,'FillType',[1 2;2 1]);
x2 = [x, fliplr(x)];
...
legend('Uncontrolled I','Controlled I','Desired', 'Vaccination', 'Isolation','FontSize',20);
set(gca,'FontSize',20);
% x4 = [x, fliplr(x)];
% inBetween = [y_curve1, fliplr(y_curve2)];
% fill(x4, inBetween, 'y');
% hold on;
...
for i=2:60
    ...
    input_vector = [I(i) - y(i) ((I(i) - y(i))-(I(i-1) - y(i-1)))]/N(i);
    input_vector(abs(input_vector)<0.01) = 0;
    evalpid_result = evalfis(vacc,input_vector);
    vacci_rate(i) = max(0,evalpid_result(1));
    isola_rate(i) = max(0,evalpid_result(2));
end

cost = 10*sqrt(sum((I-y).^2)) + sum(isola_rate((y-I)>0.01)) + sum(vacci_rate((y-I)>0.0001)) + sum(I(y==0));% + sum(abs(y-I)<0.0001); % cost function

end

function evalpid_result = evalpid(P,input_vector)
evalpid_result(1) = sum(P(1:2).*input_vector);
evalpid_result(2) = sum(P(3:4).*input_vector);
end
