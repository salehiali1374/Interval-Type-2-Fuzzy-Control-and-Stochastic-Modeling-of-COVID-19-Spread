...
for iteration=1:10
    ...
    vacc = mamfistype2("NumInputs", 2, "NumOutputs", 2, 'NumInputMFs', 2 , 'NumOutputMFs', 2, 'MFType', "gaussmf"); %,'NumInputMFs',5,'NumOutputMFs',5
    vacc.Inputs(1,1).Range = [-0.25 0.25];
    vacc.Inputs(1,2).Range = [-0.05 0.05];
    vacc.Outputs(1,1).Range = [-0.1 0.5];
    vacc.Outputs(1,2).Range = [-0.1 0.5];
...
cost = 10*sqrt(sum((I-y>0).^2)) + sum(isola_rate((y-I)>0.01)) + sum(vacci_rate((y-I)>0.0001));% + sum(abs(y-I)<0.0001); % cost function
